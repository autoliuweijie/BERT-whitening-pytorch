...
class SICKRelatednessCosinEval(SICKRelatednessEval):

    def similarity(self, e1, e2):
        calc_similarity = lambda s1, s2: np.nan_to_num(cosine(np.nan_to_num(s1), np.nan_to_num(s2)))
        all_scores = []
        for kk in range(e1.shape[0]):
            score = calc_similarity(e1[kk], e2[kk])
    ...
    def run(self, params, batcher):
        ...
        # trainA = sick_embed['train']['X_A']
        # trainB = sick_embed['train']['X_B']
        # trainY = self.sick_data['train']['y']
        # trainY_hat = self.similarity(trainA, trainB)

        # # Dev
        # devA = sick_embed['dev']['X_A']
        # devB = sick_embed['dev']['X_B']
        # devY = self.sick_data['dev']['y']
        # devY_hat = self.similarity(devA, devB)

        # Test
        testA = sick_embed['test']['X_A']
        testB = sick_embed['test']['X_B']
        testY = self.sick_data['test']['y']
        testY_hat = self.similarity(testA, testB)

        pr = pearsonr(testY_hat, testY)[0]
        sr = spearmanr(testY_hat, testY)[0]
        pr = 0 if pr != pr else pr
        sr = 0 if sr != sr else sr
        se = mean_squared_error(testY_hat, testY)
        logging.debug('Test : Pearson {0} Spearman {1} MSE {2}\n'.format(pr, sr, se))

        return {'pearson': pr, 'spearman': sr, 'mse': se, 'ntest': len(testA)}
